...
class DeploymentService {
  ...
  async deployAgent(agentId, options = {}) {
    try {
      ...
      console.log('✅ Backend API deployment response:', responseData);
  ...
  /**
   * Get deployment status and progress via backend API
   */
  async getDeploymentStatus(deploymentId) {
    try {
      // Use getDeployment method which now uses API
  ...
  async subscribeToDeploymentUpdates(agentId, onUpdate, onError = null) {
      ...
      if (!agentId || typeof agentId !== 'string' || agentId.trim() === '') {
        ...
        console.error('🚨 [DEPLOYMENT-SERVICE] Invalid agentId parameter:', { agentId, type: typeof agentId });
        if (onError) {
          onError(error);
        }
      ...
      if (this.pendingSubscriptions?.has(subscriptionKey)) {
        console.log('🔍 [DEPLOYMENT-SERVICE] Subscription already being created, waiting for completion:', agentId);
      ...
      if (this.activeSubscriptions.has(agentId)) {
        console.log('🔍 [DEPLOYMENT-SERVICE] Using existing subscription for agentId:', agentId);
        
        // Return cached data immediately if available
        if (this.deploymentsCache.has(agentId)) {
          const cachedDeployments = this.deploymentsCache.get(agentId);
          console.log('🔍 [DEPLOYMENT-SERVICE] Returning cached deployments:', cachedDeployments.length);
      ...
      console.log('🔍 [DEPLOYMENT-SERVICE] Creating new subscription for agentId:', agentId);
  ...
  async _createSubscriptionInternal(agentId, onUpdate, onError) {
      console.log('🔍 [DEPLOYMENT-SERVICE] Setting up API polling for agentId:', agentId);

      try {
        // Step 1: Load initial data from API
        console.log('🔍 [DEPLOYMENT-SERVICE] Step 1: Loading initial data from API...');

        const response = await agentBuilderApi.get(`/api/agents/${agentId}/deployments`, {
          params: { limit: 50 }
        });

        const initialDeployments = response.data.deployments || [];

        console.log('🔍 [DEPLOYMENT-SERVICE] Initial load successful:', {
          agentId,
          documentCount: initialDeployments.length
        });

        // Cache the initial deployments
        this.deploymentsCache.set(agentId, initialDeployments);

        // Immediately call onUpdate with initial data
        console.log('🔍 [DEPLOYMENT-SERVICE] Calling onUpdate with initial', initialDeployments.length, 'deployments');
        onUpdate([...initialDeployments]);

      } catch (initialError) {
        console.error('🚨 [DEPLOYMENT-SERVICE] Initial load failed:', initialError);
        if (onError) {
      ...
      // Step 2: Set up polling for real-time updates
      console.log('🔍 [DEPLOYMENT-SERVICE] Step 2: Setting up polling for future updates...');
      ...
      const pollInterval = setInterval(async () => {
        try {
          ...
          const cached = this.deploymentsCache.get(agentId);
          const hasChanges = JSON.stringify(cached) !== JSON.stringify(deployments);

          if (hasChanges) {
            console.log('🎯 [DEPLOYMENT-SERVICE] ✅ POLLING UPDATE! Data changed:', {
              agentId,
              documentCount: deployments.length,
            ...
            console.log('🔍 [DEPLOYMENT-SERVICE] Calling onUpdate with polled', deployments.length, 'deployments');
            onUpdate([...deployments]);
          }

        } catch (error) {
          console.error('🚨 [DEPLOYMENT-SERVICE] Polling ERROR:', error);
          if (onError) {
            onError(error);
          }
        }
      ...
      // Create enhanced unsubscribe function that cleans up cache and polling
      const enhancedUnsubscribe = () => {
        console.log('🔍 [DEPLOYMENT-SERVICE] Cleaning up polling for agentId:', agentId);
        // Stop polling
        clearInterval(pollInterval);
      ...
      console.log('🔍 [DEPLOYMENT-SERVICE] Polling setup complete for agentId:', agentId);
  ...
  subscribeToSingleDeployment(deploymentId, onUpdate, onError = null) {
    if (!deploymentId || typeof deploymentId !== 'string' || deploymentId.trim() === '') {
      ...
      console.error('🚨 [DEPLOYMENT-SERVICE] Invalid deploymentId parameter:', { deploymentId });
      if (onError) {
        onError(error);
      }
      return () => {}; // Return empty unsubscribe function
    ...
    console.log('🔍 [DEPLOYMENT-SERVICE] Setting up single deployment polling for:', deploymentId);
    ...
    const pollDeployment = async () => {
      try {
        ...
        if (!agentId) {
          console.warn('🚨 [DEPLOYMENT-SERVICE] Cannot poll - agent_id not found in cache for deployment:', deploymentId);
          return;
        }

        // Fetch from API
        const response = await agentBuilderApi.get(`/api/agents/${agentId}/deployments/${deploymentId}`);
        const deployment = response.data;

        // Only call onUpdate if status changed
        if (deployment.status !== lastStatus) {
          console.log('🔍 [DEPLOYMENT-SERVICE] Single deployment status changed:', {
            deployment_id: deploymentId,
        ...
        const terminalStates = ['completed', 'failed', 'cancelled', 'deployed'];
        if (terminalStates.includes(deployment.status?.toLowerCase())) {
          console.log('🔍 [DEPLOYMENT-SERVICE] Deployment reached terminal state:', deployment.status);
          clearInterval(pollInterval);
        }

      } catch (error) {
        console.error('🚨 [DEPLOYMENT-SERVICE] Single deployment polling error:', error);
        if (onError) {
          onError(error);
        }
      }
    ...
    // Set up polling (every 2 seconds for active deployment)
    const pollInterval = setInterval(pollDeployment, 2000);

    // Return unsubscribe function
    return () => {
      console.log('🔍 [DEPLOYMENT-SERVICE] Cleaning up single deployment polling for:', deploymentId);
      clearInterval(pollInterval);
    };
  }

  /**
   * Get a single deployment by ID via backend API
   */
  async getDeployment(deploymentId) {
    try {
      ...
      if (!agentId) {
        throw new Error('Deployment not found in cache. Agent ID required.');
      }
  ...
  async getAgentServerStatus(agentId) {
    try {
      ...
      const status = await dataAccess.getAgentServerStatus(agentId);

      if (!status) {
        return false;
      }

      // Agent is deployed if it's running
      return Boolean(status.isRunning && status.status === 'deployed');
  ...
  subscribeToAgentServerStatus(agentId, onUpdate, onError = null) {
    try {
      ...
      const unsubscribe = dataAccess.subscribeToAgentServerStatus(agentId, (status) => {
        if (status) {
          const isDeployed = Boolean(status.isRunning && status.status === 'active');
          onUpdate(isDeployed, status);
        } else {
          onUpdate(false, null);
        }
      });
