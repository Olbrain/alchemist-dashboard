...
class TiledeskService {
  ...
  /**
   * Get Tiledesk bot account by agent ID via backend API
   */
  async getBot(agentId) {
    try {
      ...
      const botData = await dataAccess.getTiledeskBot(agentId);

      if (!botData) {
        console.warn('[Tiledesk] No bot found for agentId:', agentId);
        return null; // Bot not found
      }

      console.log('[Tiledesk] Bot data received:', botData);
      ...
      console.error('[Tiledesk] Error getting bot:', error);
      throw error;
    }
  }
