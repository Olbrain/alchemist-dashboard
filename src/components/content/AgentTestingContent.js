...
const AgentTestingContent = ({ agentId }) => {
  ...
  useEffect(() => {
    const loadAgentAndDeploymentStatus = async () => {
      ...
      try {
        ...
        console.log('🧪 [AgentTestingContent] Checking deployment status for agentId:', agentId);
  ...
  const loadTestSessions = React.useCallback(async () => {
    try {
      // Add safety checks
      if (!selectedAgent) {
        console.warn('🧪 [loadTestSessions] No agent selected, skipping load');
        setTestSessions([]);
        return;
      }

      if (!organizationId) {
        console.warn('🧪 [loadTestSessions] Organization ID not available, skipping load');
        setTestSessions([]);
        return;
      }

      setLoadingSessions(true);
      console.log('🧪 [loadTestSessions] Fetching sessions with parameters:', {
      ...
      console.log('🧪 [loadTestSessions] Successfully loaded sessions:', {
      ...
      console.error('🧪 [loadTestSessions] Error loading test sessions:', error);
      setTestSessions([]);
    } finally {
  ...
  useEffect(() => {
    if (selectedAgent && organizationId) {
      console.log('🧪 [AgentTestingContent] Loading test sessions for:', {
        agentId: selectedAgent.id,
      ...
      console.log('🧪 [AgentTestingContent] Skipping session load:', {
